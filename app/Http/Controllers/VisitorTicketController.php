...
class VisitorTicketController extends Controller
{
    ...
    public function showReceipt($payment_code)
    {
        ...
        $eventDate = $settings['visitor_event_date'] ?? '3 - 5 November 2026';
        $eventVenue = $settings['visitor_event_venue'] ?? 'Royal Ambarrukmo Yogyakarta';
        $defaultBankInfo = "Bank : Mandiri\nAccount No. : 1030099991461\nAccount Holder : IAGI";
        $rawBankInfo = $settings['visitor_bank_transfer_info'] ?? $settings['bank_info'] ?? $defaultBankInfo;
        $bankInfo = str_replace(['137-00-1234567-8', 'Ikatan Ahli Geologi Indonesia (IAGI)'], ['1030099991461', 'IAGI'], $rawBankInfo);
        $bankInfo = preg_replace('/Mandiri\s+Bank/i', 'Bank : Mandiri', $bankInfo);
        $bankInfo = preg_replace('/Bank\s+Mandiri/i', 'Bank : Mandiri', $bankInfo);
        $bankInfo = preg_replace('/Account\s+Number\s*:/i', 'Account No. :', $bankInfo);
        $bankInfo = preg_replace('/No\.\s*Rek\s*:/i', 'Account No. :', $bankInfo);
