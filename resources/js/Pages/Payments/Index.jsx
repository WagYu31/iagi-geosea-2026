...
export default function Index({ payments = [], submissions = [], midtrans_client_key, pricing = {} }) {
    ...
    // Get fee for a submission based on its participant_category
    const getSubmissionFee = (submission) => {
        const cat = (submission?.participant_category || '').toLowerCase();
        return pricing[cat] || null;
    };
    ...
    const handleMidtransPayment = async () => {
        ...
        if (!fee) {
            setSnackbar({ open: true, message: 'Unable to determine fee. Category not recognized.', severity: 'warning' });
            return;
        }
        ...
        try {
            ...
            if (window.snap) {
                window.snap.pay(data.snap_token, {
                    ...
                    onPending: () => { setSnackbar({ open: true, message: '⏳ Payment pending. Please complete your payment.', severity: 'info' }); setTimeout(() => router.reload(), 1500); },
                    onError: () => { setSnackbar({ open: true, message: '❌ Payment failed. Please try again.', severity: 'error' }); setTimeout(() => router.reload(), 1500); },
    ...
    // Selected submission's info
    const selectedFee = selectedSubmission ? getSubmissionFee(selectedSubmission) : null;
    const selectedCatConfig = selectedSubmission ? getSubmissionCategory(selectedSubmission) : null;
    const selectedCatLabel = selectedCatConfig?.label || selectedSubmission?.participant_category || 'Unknown';
            ...
            <Box component="main" sx={{ p: { xs: 2, sm: 3, md: 3.5 }, maxWidth: '1200px', margin: '0 auto' }}>
                ...
                <Box sx={{
                    ...
                    <Box sx={{ position: 'relative', zIndex: 1 }}>
                        ...
                        <Typography sx={{ color: c.textMuted, fontSize: '0.9rem', maxWidth: 500 }}>
                ...
                <Box sx={{
                    ...
                    {Object.entries(pricing).map(([category, amount]) => {
                        ...
                        // Highlight card if any submission needing payment has this category
                        const hasMatchingSubmission = submissionsNeedingPayment.some(
                            s => (s.participant_category || '').toLowerCase() === category
                        );
                        return (
                            <Card key={category} elevation={0} sx={{
                                borderRadius: '18px', position: 'relative', overflow: 'hidden',
                                border: hasMatchingSubmission
                                    ? `2px solid ${config.color}`
                                    : `1px solid ${isDark ? '#2a2a2a' : '#e5e7eb'}`,
                                bgcolor: isDark ? '#1a1a1a' : '#fff',
                                transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                                transform: hasMatchingSubmission ? 'scale(1.02)' : 'scale(1)',
                                '&:hover': {
                                    transform: hasMatchingSubmission ? 'scale(1.03)' : 'scale(1.01)',
                                ...
                                {hasMatchingSubmission && (
                                    <Box sx={{
                                        ...
                                        px: 1.2, py: 0.3, borderRadius: '6px', background: config.gradient,
                                ...
                                <CardContent sx={{ p: 3, pt: 2.5 }}>
                                    ...
                                    <Typography sx={{ fontWeight: 700, fontSize: '0.85rem', color: c.textPrimary, mb: 0.3, lineHeight: 1.3 }}>
                                        {config.label || category}
                                    </Typography>
                                    <Typography sx={{ fontSize: '0.72rem', color: c.textMuted, mb: 2 }}>Registration fee</Typography>
                                    <Typography sx={{
                                        fontWeight: 900, fontSize: '1.5rem', letterSpacing: '-0.02em',
                                        color: hasMatchingSubmission ? config.color : c.textPrimary,
                    ...
                    <Paper elevation={0} sx={{
                        ...
                        <Stack spacing={0} divider={<Divider sx={{ borderColor: isDark ? 'rgba(245,158,11,0.1)' : '#fef3c7' }} />}>
                            {submissionsNeedingPayment.map((submission) => {
                                const fee = getSubmissionFee(submission);
                                const catConfig = getSubmissionCategory(submission);
                                return (
                                    <Box key={submission.id} sx={{
                                        ...
                                        <Box sx={{ flex: 1, minWidth: 0 }}>
                                            ...
                                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, flexWrap: 'wrap' }}>
                                                {catConfig && (
                                                    <Chip
                                                        icon={React.cloneElement(catConfig.icon, { sx: { fontSize: '14px !important' } })}
                                                        label={catConfig.shortLabel}
                                                        size="small"
                                                        sx={{
                                                            fontWeight: 600, fontSize: '0.68rem', borderRadius: '6px', height: 24,
                                                            bgcolor: isDark ? `${catConfig.color}15` : `${catConfig.color}10`,
                                                            color: isDark ? catConfig.lightColor : catConfig.color,
                                                            '& .MuiChip-icon': { color: isDark ? catConfig.lightColor : catConfig.color },
                                                        }}
                                                    />
                                                )}
                                                {fee && (
                                                    <Typography sx={{ fontSize: '0.85rem', fontWeight: 800, color: '#1abc9c', fontFamily: 'monospace' }}>
                                                        {formatRp(fee)}
                                                    </Typography>
                                                )}
                                            </Box>
                ...
                {/* ─── PAYMENT HISTORY ─── */}
                <Paper elevation={0} sx={{ borderRadius: '18px', overflow: 'hidden', border: `1px solid ${c.cardBorder}`, bgcolor: c.cardBg }}>
                    <Box sx={{ px: 3, py: 2.5, borderBottom: `1px solid ${c.cardBorder}`, display: 'flex', alignItems: 'center', gap: 1.5 }}>
                        <Avatar sx={{ width: 32, height: 32, borderRadius: '8px', bgcolor: isDark ? 'rgba(26,188,156,0.12)' : '#f0fdf9' }}>
                            <ReceiptLongIcon sx={{ color: '#1abc9c', fontSize: 18 }} />
                        </Avatar>
                        <Typography sx={{ fontWeight: 800, fontSize: '1rem', color: c.textPrimary }}>Payment History</Typography>
                        {payments.length > 0 && (
                            <Chip label={payments.length} size="small" sx={{ height: 22, fontWeight: 700, fontSize: '0.7rem', bgcolor: isDark ? 'rgba(26,188,156,0.12)' : '#ecfdf5', color: '#1abc9c', borderRadius: '6px' }} />
                        )}
                    </Box>
                        ...
                        <Box sx={{ textAlign: 'center', py: { xs: 6, md: 10 }, px: 3 }}>
                            <Box sx={{ position: 'relative', display: 'inline-block', mb: 3.5 }}>
                                <Box sx={{
                                    ...
                                    background: isDark ? 'linear-gradient(135deg, rgba(26,188,156,0.08) 0%, rgba(26,188,156,0.03) 100%)' : 'linear-gradient(135deg, #f0fdf9 0%, #ecfdf5 100%)',
                            ...
                            <Typography sx={{ fontWeight: 800, color: c.textPrimary, fontSize: '1.15rem', mb: 0.8, letterSpacing: '-0.01em' }}>No Payments Yet</Typography>
                                        ...
                                        <TableHead>
                                            <TableRow>
                                                {['Submission', 'Category', 'Amount', 'Method', 'Status', 'Date', ''].map(h => (
                                                    <TableCell key={h} sx={{ fontWeight: 700, fontSize: '0.7rem', color: c.textMuted, textTransform: 'uppercase', letterSpacing: '0.06em', py: 1.5, borderBottom: `1px solid ${c.cardBorder}`, bgcolor: isDark ? 'rgba(0,0,0,0.15)' : '#fafafa' }}>{h}</TableCell>
                                                ))}
                                            </TableRow>
                                        </TableHead>
                                        <TableBody>
                                            {payments.map((payment) => {
                                                const pCatConfig = categoryConfig[(payment.submission?.participant_category || '').toLowerCase()];
                                                return (
                                                    <TableRow key={payment.id} sx={{
                                                        ...
                                                        <TableCell>
                                                            {pCatConfig ? (
                                                                <Chip
                                                                    icon={React.cloneElement(pCatConfig.icon, { sx: { fontSize: '13px !important' } })}
                                                                    label={pCatConfig.shortLabel}
                                                                    size="small"
                                                                    sx={{
                                                                        fontWeight: 600, fontSize: '0.65rem', borderRadius: '6px', height: 22,
                                                                        bgcolor: isDark ? `${pCatConfig.color}15` : `${pCatConfig.color}10`,
                                                                        color: isDark ? pCatConfig.lightColor : pCatConfig.color,
                                                                        '& .MuiChip-icon': { color: isDark ? pCatConfig.lightColor : pCatConfig.color },
                                                                    }}
                                                                />
                                                            ) : (
                                        ...
                                        return (
                                            <Card key={payment.id} variant="outlined" sx={{
                                                ...
                                                <CardContent sx={{ p: 2 }}>
                                                    ...
                                                    <Typography sx={{ fontWeight: 600, fontSize: '0.85rem', color: c.textPrimary, mb: 0.5 }}>{payment.submission?.title || 'N/A'}</Typography>
                                                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1.5 }}>
                                                        {pCatConfig && (
                                                            <Chip icon={React.cloneElement(pCatConfig.icon, { sx: { fontSize: '12px !important' } })} label={pCatConfig.shortLabel} size="small"
                                                                sx={{ fontWeight: 600, fontSize: '0.65rem', borderRadius: '6px', height: 22, bgcolor: isDark ? `${pCatConfig.color}15` : `${pCatConfig.color}10`, color: isDark ? pCatConfig.lightColor : pCatConfig.color, '& .MuiChip-icon': { color: isDark ? pCatConfig.lightColor : pCatConfig.color } }} />
                                                        )}
            ...
            <Dialog open={openDialog} onClose={handleCloseDialog} maxWidth="sm" fullWidth
                ...
                <Box sx={{
                    ...
                    <Box sx={{ position: 'relative', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        <Box>
                            ...
                            <Typography sx={{ fontSize: '0.78rem', color: 'rgba(255,255,255,0.7)' }}>Your payment is protected by Midtrans</Typography>
                        </Box>
                        ...
                        <>
                            ...
                            <Box sx={{
                                ...
                                {[
                                    ...
                                    { label: 'Category', value: selectedCatLabel, chip: true },
                                    { label: 'Registration Fee', value: selectedFee ? formatRp(selectedFee) : '—' },
                                ].map((item) => (
                                    <Box key={item.label} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1.2 }}>
                                        ...
                                        {item.chip && selectedCatConfig ? (
                                            <Chip
                                                icon={React.cloneElement(selectedCatConfig.icon, { sx: { fontSize: '13px !important' } })}
                                                label={item.value} size="small"
                                                sx={{
                                                    fontWeight: 600, fontSize: '0.7rem', borderRadius: '6px', height: 24,
                                                    bgcolor: isDark ? `${selectedCatConfig.color}15` : `${selectedCatConfig.color}10`,
                                                    color: isDark ? selectedCatConfig.lightColor : selectedCatConfig.color,
                                                    '& .MuiChip-icon': { color: isDark ? selectedCatConfig.lightColor : selectedCatConfig.color },
                                                }}
                                            />
                                        ) : (
                                ...
                                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    ...
                                    <Typography sx={{ fontSize: '1.4rem', fontWeight: 900, color: '#1abc9c', fontFamily: '"Inter", monospace', letterSpacing: '-0.02em' }}>
                                        {selectedFee ? formatRp(selectedFee) : '—'}
                                    </Typography>
                                </Box>
                            </Box>

                            {/* Payment method icons */}
                            <Box sx={{ display: 'flex', justifyContent: 'center', gap: { xs: 1.5, sm: 2.5 }, py: 1, flexWrap: 'wrap' }}>
                                {[
                                    ...
                                    <Box key={m.label} sx={{ textAlign: 'center' }}>
                                        <Avatar sx={{
                                            ...
                                            border: `1px solid ${isDark ? '#333' : '#e5e7eb'}`, fontSize: m.isEmoji ? 18 : 'inherit',
                                        }}>
                ...
                <DialogActions sx={{ px: 3, pb: 3, pt: 1, gap: 1 }}>
                    <Button onClick={handleCloseDialog} disabled={paymentLoading}
                        sx={{ color: c.textMuted, textTransform: 'none', fontWeight: 600, borderRadius: '12px', px: 3, fontSize: '0.85rem' }}>Cancel</Button>
                    <Button variant="contained" onClick={handleMidtransPayment}
                        disabled={paymentLoading || !selectedFee}
                    ...
                    >
                        {paymentLoading ? 'Processing...' : `Pay ${selectedFee ? formatRp(selectedFee) : ''}`}
                    </Button>
                </DialogActions>
            </Dialog>
